...
const siteConfig = {
  images: {
    hod: 'images/hod.jpg',
    about1: 'images/Electrical-block.jpg',
    about2: 'images/Electric1.jpg',
    opportunities: 'images/Electric2.png'
  },
  pages: {
...
function applySiteConfig() {
  ...
  setImage('hodImage', siteConfig.images.hod);
  setImage('aboutImage1', siteConfig.images.about1);
  setImage('aboutImage2', siteConfig.images.about2);
  setImage('opportunitiesImage', siteConfig.images.opportunities);
  ...
  setLink('linkSyllabus', siteConfig.pages.syllabus);
  setLink('linkNotes', siteConfig.pages.notes);
  setLink('linkEvents', siteConfig.pages.events);
  setLink('linkProjects', siteConfig.pages.projects);
  setLink('linkGallery', siteConfig.pages.gallery);
  setLink('linkActivities', siteConfig.pages.activities);
  setLink('linkAnnouncements', siteConfig.pages.announcements);
  setLink('linkPlacements', siteConfig.pages.placements);
}

window.addEventListener('DOMContentLoaded', applySiteConfig, false);
...
function closeMenu() {
  if (!menuOpen || !toggle || !menu) return;
  menuOpen = false;
...
function animateThemeSwitch(nextDark) {
  ...
  themeOverlay.style.setProperty('--overlay-x', `${rect.left + rect.width / 2}px`);
  themeOverlay.style.setProperty('--overlay-y', `${rect.top + rect.height / 2}px`);
  themeOverlay.style.background = nextDark
    ? 'radial-gradient(circle at center, rgba(11, 19, 32, 0.96) 0%, rgba(11, 19, 32, 0.45) 30%, transparent 70%)'
    : 'radial-gradient(circle at center, rgba(255, 255, 255, 0.96) 0%, rgba(255, 255, 245, 0.8) 35%, transparent 70%)';
  themeToggle.classList.toggle('flipped', !nextDark);
  themeOverlay.classList.remove('animate');
  void themeOverlay.offsetWidth;
  themeOverlay.classList.add('animate');
  clearTimeout(themeOverlay._themeTimeout);
  themeOverlay._themeTimeout = setTimeout(() => themeOverlay.classList.remove('animate'), 1100);
  setTimeout(() => {
    applyTheme(nextDark);
